...
class Environment(gym.Env):
    ...
    def __init__(self,
                 ...
                 min_area: Optional[float] = 150. * 150.,  # nm^2
                 num_flights: int = 15,
                 ...
                 num_detection_sectors: Optional[int] = 36,
        ...
        self.intention_matrix = np.array([_ for _ in range(9)]).reshape([3, 3])
    ...
    def resolution(self, action: List, t) -> None:
        ...
        for i in range(self.num_flights):
            if i not in self.done:
                ...
                self.flights[i].intention_airspeed, self.flights[i].intention_heading \
                ...
                self.flights[i].airspeed = \
                    self.flights[i].optimal_airspeed + self.airspeed_change * self.flights[i].last_intention_airspeed +\
                    self.airspeed_change * (t + 1) / self.num_move * (self.flights[i].intention_airspeed - self.flights[
    ...
    def reward(self) -> List:
        ...
        the reward considers: conflict, warning, economy [, smoothness]
        """
        ...
        for i in range(self.num_flights):
            if i in self.done:
                ...
                # reward
                r = 100 * (1 + p_conflict) * self.flights[i].r_conflict \
                    + 10 * (1 + p_warning) * self.flights[i].r_warning \
                    + 1 * self.flights[i].r_eco_airspeed + 5 * self.flights[i].r_eco_heading
                reward.append(r)
        return reward
    ...
    def observation(self) -> List:
        ...
        intruder info (a sector of detection area):
            * using polar coordinates (origin: owner's position, x-axis: owner's bearing)
            ...
            intention: bearing, distance to target
        """
        ...
        for i in range(self.num_flights):
            if i in self.done:
                ...
                intruder_info = np.zeros([self.num_detection_sectors, 3])
                for j in range(self.num_flights):
                    if j != i and j not in self.done:
                        ...
                        if distance_i_j <= self.range_detection:
                            # intruder info:
                            ...
                            if intrusion_distance > intruder_info[sector_idx, 1]:
                                intruder_info[sector_idx, 0] = 1.  # occupation label
                                intruder_info[sector_idx, 1] = intrusion_distance  # intrusion distance
                                intruder_info[sector_idx, 2] = angel_clip(
                                    self.flights[j].bearing - self.flights[i].bearing)  # bearing

                ob = intruder_info.flatten()
    ...
    def reset(self) -> List:
        ...
        min_distance_ = self.distance_init_buffer * self.min_distance
        while len(self.flights) < self.num_flights:
            ...
            for f in self.flights:
                if candidate.position.distance(f.position) < min_distance_ or candidate.target.distance(
                        f.target) < min_distance_:
                    valid = False
                    break
            if valid:
