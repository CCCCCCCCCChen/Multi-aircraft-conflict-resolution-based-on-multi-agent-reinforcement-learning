"""
Used for training agents for 3D scenarios
"""
...
if __name__ == "__main__":
    ...
    N_STATES = 3 * 12 * 3 + 3  # number of neurons in the input layer
    N_HIDDEN = 18 * 12  # number of neurons in the hidden layer
    N_ACTION = 3 * 3 * 3  # number of neurons in the output layer
    ...
    for episode in tqdm(range(args.episodes)):
            ...
            for i in range(env.num_flights):
                if i not in env.done:
                    ...
                    if env.flights[i].r_eco_airspeed == -1:
                        eco_airspeed -= 1
                    eco_level += env.flights[i].r_eco_level
                    if env.flights[i].r_smo_airspeed == -1:
                        smo_airspeed -= 1
                    if env.flights[i].r_smo_heading == -1:
                        smo_heading -= 1
                    if env.flights[i].r_smo_level == -1:
                        smo_level -= 1
                    if env.flights[i].r_over_max_level == -1 or env.flights[i].r_over_min_level == -1:
                        number_illegal_actions += 1
        ...
        if episode % SHOW_ITER == 0:
            writer.add_scalar('3D_training/01_num_conflicts', number_conflict, episode)
            writer.add_scalar('3D_training/02_num_warnings', number_warning, episode)
            writer.add_scalar('3D_training/03_total_reward', total_rew, episode)
            writer.add_scalar('3D_training/04_average_reward_per_action', total_rew / num_rew, episode)
            writer.add_scalar('3D_training/05_num_illegal_actions', number_illegal_actions, episode)
            writer.add_scalar('3D_training/06_rate_of_total_extra_flight_distance',
                              total_trajectory_length / total_optimal_trajectory_length, episode)
            writer.add_scalar('3D_training/07_economy_of_heading', eco_heading, episode)
            writer.add_scalar('3D_training/08_economy_of_airspeed', eco_airspeed, episode)
            writer.add_scalar('3D_training/09_economy_of_level', eco_level, episode)
            writer.add_scalar('3D_training/10_smoothness_of_heading', smo_heading, episode)
            writer.add_scalar('3D_training/11_smoothness_of_airspeed', smo_airspeed, episode)
            writer.add_scalar('3D_training/12_smoothness_of_level', smo_level, episode)
